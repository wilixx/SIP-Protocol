...
class registerar(server):
    ...
    def __init__(self, server_name, domain, server_network_name, content_type, content_sub_type, protocol='TCP', port=6050):
        self._initialize_db()
        self._set_server_name(server_name)
        self._set_domain(domain)
        self._set_set_server_network_name(server_network_name)
        self._set_content_type(content_type)
        self._set_content_sub_type(content_sub_type)
    ...
    def _register_server(self, client_socket):
        msg = client_socket.recv(self.__s._get_buff_size()).decode('UTF-8')
        print(msg)
        if msg[:8] == 'REGISTER':
    ...
    def _establish_session(self, client_socket, seq_num, client_name, client_network_name):
        invite_packet_a = client_socket.recv(self._s._get_buff_size())
    ...
    def _set_set_server_network_name(self, server_network_name):
        self.__server_network_name = server_network_name

    def _get_server_network_name(self):
    ...
    def _set_content_sub_type(self, content_sub_type):
        self.content_sub_type = content_sub_type

    def _get_content_sub_type(self):
...
r = registerar('registerar', 'vaaan.com', 'REGISTERAR', 'content', 'mp3')